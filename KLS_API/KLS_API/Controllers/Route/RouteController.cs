        ...
        public ActionResult DT()
        {
            try
            {
                List<DTModel> rutas = _context.Ruta.Where(x => x.estatus == 1).Select(x => new DTModel
                {
                    Id = x.id,
                    Folio = x.Folio,
                    EstadoOrigen = _context.Cat_Estado.FirstOrDefault(y => y.id == x.id_estadoorigen).nombre,
                    CiudadOrigen = _context.Cat_Ciudad.FirstOrDefault(y => y.id == x.id_ciudadorigen).nombre,
                    EstadoDestino = _context.Cat_Estado.FirstOrDefault(y => y.id == x.id_estadodestino).nombre,
                    CiudadDestino = _context.Cat_Ciudad.FirstOrDefault(y => y.id == x.id_ciudaddestino).nombre,
